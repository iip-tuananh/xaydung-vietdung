...
class PageController extends Controller
{
    ...
    public function postcontact(Request $request){
        ...
        if (isset($request->mess)) {
            ...
            $data->email = $request->email;
            $data->phone = $request->phone;
            $data->mess = $request->chieu_ngang. '*' .$request->chieu_sau. ', Số tầng: '. $request->so_tang.', Phong cách thiết kế: '. $request->phong_cach;
            $data->save();
        }
    ...
    {
        $data['duan'] = Project::where('status',1)->paginate(8);
